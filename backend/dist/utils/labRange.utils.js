...
exports.computeAbnormalFlag = computeAbnormalFlag;
function computeAbnormalFlag(value, low, high) {
    ...
    if (hasLow && hasHigh && value >= low && value <= high)
        return 'normal';
    if (hasHigh && !hasLow && value <= high)
        return 'normal';
    if (hasLow && !hasHigh && value >= low)
        return 'normal';
    return 'unknown';
}
