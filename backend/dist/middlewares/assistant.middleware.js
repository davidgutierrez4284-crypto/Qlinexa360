...
exports.AssistantMiddleware = void 0;
...
class AssistantMiddleware {
    ...
    static checkAssistantModulePermission(module) {
        return async (req, res, next) => {
            try {
                ...
                if (userRole === 'ASISTENTE') {
                    ...
                    const doctorId = req.headers['x-selected-doctor-id'] || req.user.doctorId;
                    if (!doctorId) {
                        throw new error_utils_1.AppError('Asistente no vinculado a ningún doctor. Por favor selecciona un doctor.', 403);
                    }
                    ...
                    if (!link) {
                        throw new error_utils_1.AppError('Asistente no autorizado', 403);
                    }
                    ...
                    if (!hasPermission) {
                        throw new error_utils_1.AppError(`No tienes permisos para acceder al módulo: ${module}`, 403);
                    }
                    return next();
                }
    ...
    static checkAssistantDataAccess() {
        return async (req, res, next) => {
            ...
            try {
                ...
                if (userRole === 'ASISTENTE') {
                    // Obtener doctorId del header o del token
                    const linkedDoctorId = req.headers['x-selected-doctor-id'] || req.user.doctorId;
                    if (!linkedDoctorId) {
                        throw new error_utils_1.AppError('Asistente no vinculado a ningún doctor. Por favor selecciona un doctor.', 403);
                    }
                    // Verificar que el recurso pertenece al doctor vinculado
                    const resourceDoctorId = req.params.doctorId || ((_a = req.body) === null || _a === void 0 ? void 0 : _a.doctorId);
                    if (resourceDoctorId && resourceDoctorId !== linkedDoctorId) {
                        throw new error_utils_1.AppError('No tienes permisos para acceder a esta información', 403);
                    }
                    return next();
                }
                // Si no es doctor ni asistente, denegar acceso
            ...
            try {
                ...
                if (userRole === 'ASISTENTE') {
                    ...
                    if (req.body && doctorId) {
                        req.body.realizadoPor = userId;
                        req.body.vinculadoADoctor = doctorId;
                    }
                }
                next();
            }
    ...
    static doctorOnly() {
        return async (req, res, next) => {
            try {
                ...
                const userRole = req.user.role;
                if (userRole !== 'DOCTOR') {
                    throw new error_utils_1.AppError('Solo los doctores pueden gestionar asistentes', 403);
                }
                next();
            }
            catch (error) {
...
exports.AssistantMiddleware = AssistantMiddleware;
