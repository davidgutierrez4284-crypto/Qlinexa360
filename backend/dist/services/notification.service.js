...
exports.NotificationService = exports.EmailService = exports.WhatsAppService = void 0;
...
class WhatsAppService {
    ...
    async sendInvitationMessage(phone, doctorName, invitationUrl) {
        try {
            if (!this.apiKey || !this.phoneNumberId) {
                logger_utils_1.securityLogger.warn('WhatsApp credentials not configured, skipping message');
                return false;
            }
...
Equipo Qlinexa360`;
            ...
            if (response.ok) {
                logger_utils_1.securityLogger.info(`WhatsApp invitation sent to ${phone}`);
                return true;
            }
            else {
                logger_utils_1.securityLogger.error(`Failed to send WhatsApp message to ${phone}: ${response.statusText}`);
                return false;
            }
        }
        catch (error) {
            logger_utils_1.securityLogger.error(`Error sending WhatsApp message to ${phone}:`, error);
            return false;
        }
    }
    async sendAppointmentConfirmationMessage(phone, data) {
        try {
            if (!this.apiKey || !this.phoneNumberId) {
                logger_utils_1.securityLogger.warn('WhatsApp credentials not configured, skipping message');
                return false;
            ...
            const dateStr = (0, date_utils_1.formatAppointmentDateShort)(data.date, tz);
            const timeStr = data.time || (0, date_utils_1.formatAppointmentTimeWithAmPm)(data.date, tz);
            ...
            if (response.ok)
                return true;
            logger_utils_1.securityLogger.error(`Failed to send WhatsApp appointment confirmation to ${phone}: ${response.statusText}`);
            return false;
        }
        catch (error) {
            logger_utils_1.securityLogger.error(`Error sending WhatsApp appointment confirmation to ${phone}:`, error);
            return false;
        }
    }
    async sendAppointmentCancellationMessage(phone, data) {
        try {
            if (!this.apiKey || !this.phoneNumberId) {
                logger_utils_1.securityLogger.warn('WhatsApp credentials not configured, skipping message');
                return false;
            ...
            if (response.ok)
                return true;
            logger_utils_1.securityLogger.error(`Failed to send WhatsApp appointment cancellation to ${phone}: ${response.statusText}`);
            return false;
        }
        catch (error) {
            logger_utils_1.securityLogger.error(`Error sending WhatsApp appointment cancellation to ${phone}:`, error);
            return false;
        }
    }
    async sendDoctorNotificationMessage(phone, data) {
        // For now reuse the same template
        return this.sendAppointmentConfirmationMessage(phone, data);
    }
    async sendPreConsultationLinkMessage(phone, doctorName, appointmentDate, link) {
        try {
            if (!this.apiKey || !this.phoneNumberId) {
                logger_utils_1.securityLogger.warn('WhatsApp credentials not configured, skipping message');
                return false;
...
Equipo Qlinexa360`;
            ...
            if (response.ok) {
                logger_utils_1.securityLogger.info(`WhatsApp pre-consultation link sent to ${phone}`);
                return true;
            }
            else {
                logger_utils_1.securityLogger.error(`Failed to send WhatsApp pre-consultation link to ${phone}: ${response.statusText}`);
                return false;
            }
        }
        catch (error) {
            logger_utils_1.securityLogger.error(`Error sending WhatsApp pre-consultation link to ${phone}:`, error);
            return false;
        }
    }
    async sendPreConsultationCompletedMessage(phone, patientName, appointmentDate) {
        try {
            if (!this.apiKey || !this.phoneNumberId) {
                logger_utils_1.securityLogger.warn('WhatsApp credentials not configured, skipping message');
                return false;
...
Equipo Qlinexa360`;
            ...
            if (response.ok) {
                logger_utils_1.securityLogger.info(`WhatsApp pre-consultation completed notification sent to ${phone}`);
                return true;
            }
            else {
                logger_utils_1.securityLogger.error(`Failed to send WhatsApp pre-consultation completed notification to ${phone}: ${response.statusText}`);
                return false;
            }
        }
        catch (error) {
            logger_utils_1.securityLogger.error(`Error sending WhatsApp pre-consultation completed notification to ${phone}:`, error);
            return false;
        }
    }
}
...
class EmailService {
    ...
    async sendInvitationEmail(email, doctorName, invitationUrl) {
        try {
      ...
      `;
            const ok = await (0, email_utils_1.sendEmailHtml)(email, subject, htmlContent, email_utils_1.fromAddresses.noReply);
            if (ok)
                logger_utils_1.securityLogger.info(`Email invitation sent to ${email}`);
            else
                logger_utils_1.securityLogger.error(`Failed to send email to ${email}`);
            return ok;
        }
        catch (error) {
            logger_utils_1.securityLogger.error(`Error sending email to ${email}:`, error);
            return false;
        }
    }
    /**
    ...
    async sendPatientAvisoPrivacidadToDoctor(params) {
      ...
      `;
            const attachments = [
                { filename: 'Aviso_Privacidad_Paciente.pdf', content: params.avisoPdfBuffer, contentType: 'application/pdf' }
            ];
            const ok = await (0, email_utils_1.sendEmailHtml)(params.doctorEmail, subject, htmlContent, email_utils_1.fromAddresses.noReply, attachments);
            if (ok)
                logger_utils_1.securityLogger.info(`Patient Aviso de Privacidad sent to doctor ${params.doctorEmail} for patient ${params.patientEmail}`);
            else
                logger_utils_1.securityLogger.error(`Failed to send patient Aviso de Privacidad to doctor ${params.doctorEmail}`);
            return ok;
        }
        catch (error) {
            logger_utils_1.securityLogger.error('Error sending patient Aviso de Privacidad to doctor:', error);
            return false;
        }
    }
    async sendPreConsultationLinkEmail(email, doctorName, appointmentDate, link) {
        try {
      ...
      `;
            const ok = await (0, email_utils_1.sendEmailHtml)(email, subject, htmlContent, email_utils_1.fromAddresses.noReply);
            if (ok)
                logger_utils_1.securityLogger.info(`Pre-consultation link email sent to ${email}`);
            else
                logger_utils_1.securityLogger.error(`Failed to send pre-consultation link email to ${email}`);
            return ok;
        }
        catch (error) {
            logger_utils_1.securityLogger.error(`Error sending pre-consultation link email to ${email}:`, error);
            return false;
        }
    }
    async sendPreConsultationCompletedEmail(email, patientName, appointmentDate, timezone) {
        try {
            const subject = 'Pre-Consulta Completada - Qlinexa360';
            const appointmentDateStr = `${(0, date_utils_1.formatAppointmentDate)(appointmentDate, timezone)} ${(0, date_utils_1.formatAppointmentTime)(appointmentDate, timezone)}`;
            const htmlContent = `
        <!DOCTYPE html>
        <html>
        <head>
      ...
      `;
            const ok = await (0, email_utils_1.sendEmailHtml)(email, subject, htmlContent, email_utils_1.fromAddresses.noReply);
            if (ok)
                logger_utils_1.securityLogger.info(`Pre-consultation completed notification email sent to ${email}`);
            else
                logger_utils_1.securityLogger.error(`Failed to send pre-consultation completed email to ${email}`);
            return ok;
        }
        catch (error) {
            logger_utils_1.securityLogger.error(`Error sending pre-consultation completed email to ${email}:`, error);
            return false;
        }
    }
    async sendAssistantInvitationEmail(email, doctorName, invitationUrl) {
        try {
      ...
      `;
            const ok = await (0, email_utils_1.sendEmailHtml)(email, subject, htmlContent, email_utils_1.fromAddresses.noReply);
            if (ok)
                logger_utils_1.securityLogger.info(`Assistant invitation email sent to ${email}`);
            else
                logger_utils_1.securityLogger.error(`Failed to send assistant invitation email to ${email}`);
            return ok;
        }
        catch (error) {
            logger_utils_1.securityLogger.error(`Error sending assistant invitation email to ${email}:`, error);
            return false;
        }
    }
    async sendExternalCollaborationInviteEmail(email, inviterName, patientName, padecimiento, websiteUrl, inviterEmail, avisoPdfBuffer) {
        try {
      ...
      `;
            const attachments = avisoPdfBuffer
                ? [{ filename: `Aviso_Privacidad_Paciente_${patientName.replace(/\s+/g, '_')}.pdf`, content: avisoPdfBuffer, contentType: 'application/pdf' }]
                : undefined;
            const sent = await (0, email_utils_1.sendEmailHtml)(email, subject, htmlContent, email_utils_1.fromAddresses.noReply, attachments);
            if (!sent) {
                logger_utils_1.securityLogger.error(`Failed to send external collaboration invite email to ${email}`);
                return false;
            }
            if (inviterEmail) {
        ...
        `;
                ...
                if (copyOk) {
                    ...
                    logger_utils_1.securityLogger.error(`Failed to send copy of external collaboration invite email to ${inviterEmail}`);
                }
            }
            logger_utils_1.securityLogger.info(`External collaboration invite email sent to ${email}`);
            return true;
        }
        catch (error) {
            logger_utils_1.securityLogger.error(`Error sending external collaboration invite email to ${email}:`, error);
            return false;
        }
    }
    async sendInternalCollaborationInviteEmail(email, inviterName, patientName, padecimiento, websiteUrl, avisoPdfBuffer) {
        try {
      ...
      `;
            ...
            const sent = await (0, email_utils_1.sendEmailHtml)(email, subject, htmlContent, email_utils_1.fromAddresses.noReply, attachments);
            if (!sent) {
                logger_utils_1.securityLogger.error(`Failed to send internal collaboration invite email to ${email}`);
                return false;
            }
            logger_utils_1.securityLogger.info(`Internal collaboration invite email sent to ${email}`);
            return true;
        }
        catch (error) {
            logger_utils_1.securityLogger.error(`Error sending internal collaboration invite email to ${email}:`, error);
            return false;
        }
    }
    async sendAppointmentConfirmationEmail(email, data) {
        try {
            const subject = 'Confirmación de cita - Qlinexa360';
            const tz = data.timezone;
            const dateStr = (0, date_utils_1.formatAppointmentDateShort)(new Date(data.date), tz);
            const htmlContent = `
        <html><body style="font-family: Arial, sans-serif; color: #333;">
          ...
          <ul>
            ...
            <li><strong>Horario:</strong> ${data.time || (0, date_utils_1.formatAppointmentTimeWithAmPm)(data.date, tz)}</li>
          ...
          <p>Gracias por usar Qlinexa360.</p>
            ...
            if (ok)
                return true;
            logger_utils_1.securityLogger.error(`Failed to send appointment confirmation email to ${email}`);
            return false;
        }
        catch (error) {
    ...
    async sendAppointmentReminderEmail(email, data) {
        try {
            const subject = `Recordatorio de cita${data.reminderLabel ? ` (${data.reminderLabel})` : ''} - Qlinexa360`;
            const tz = data.timezone;
            const dateStr = (0, date_utils_1.formatAppointmentDate)(data.date, tz);
          ...
          <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <div style="padding: 30px; background: #f8f9fa; border-radius: 0 0 8px 8px;">
              ...
              <div style="background: white; border-left: 4px solid #f59e0b; padding: 20px; margin: 20px 0; border-radius: 4px;">
                ...
                <p style="color: #555; margin: 5px 0; font-size: 16px;">
                  <strong>Hora:</strong> ${data.time || (0, date_utils_1.formatAppointmentTimeWithAmPm)(data.date, tz)}
                </p>
                ${data.reason ? `
                <p style="color: #555; margin: 5px 0; font-size: 16px;">
            ...
            if (ok)
                return true;
            logger_utils_1.securityLogger.error(`Failed to send appointment reminder email to ${email}`);
            return false;
        }
        catch (error) {
            logger_utils_1.securityLogger.error(`Error sending appointment reminder email to ${email}:`, error);
            return false;
        }
    }
    async sendAppointmentCancellationEmail(email, data) {
        try {
        ...
        <html><body style="font-family: Arial, sans-serif; color: #333;">
          <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            if (ok)
                return true;
            logger_utils_1.securityLogger.error(`Failed to send appointment cancellation email to ${email}`);
            return false;
        }
        catch (error) {
            logger_utils_1.securityLogger.error(`Error sending appointment cancellation email to ${email}:`, error);
            return false;
        }
    }
    async sendDoctorNotificationEmail(email, data) {
        try {
        ...
        <html><body style="font-family: Arial, sans-serif; color: #333;">
          ...
          <ul>
            ...
            if (ok)
                return true;
            logger_utils_1.securityLogger.error(`Failed to send doctor notification email to ${email}`);
            return false;
        }
        catch (error) {
            logger_utils_1.securityLogger.error(`Error sending doctor notification email to ${email}:`, error);
            return false;
        }
    }
    ...
    async sendCalendarEventEmail(patientEmail, data) {
        ...
        <body style="font-family: Arial, sans-serif; color: #333; line-height: 1.6; max-width: 600px; margin: 0 auto;">
          ...
          <div style="padding: 30px; background: #f8f9fa; border-radius: 0 0 10px 10px;">
            ...
            logger_utils_1.securityLogger.error(`Error sending calendar event email to ${patientEmail}:`, error);
            return false;
        }
    }
    async sendDoctorWelcomeEmail(email, firstName, lastName) {
        try {
      ...
      `;
            const ok = await (0, email_utils_1.sendEmailHtml)(email, subject, htmlContent, email_utils_1.fromAddresses.noReply);
            if (ok)
                logger_utils_1.securityLogger.info(`Doctor welcome email sent to ${email}`);
            else
                logger_utils_1.securityLogger.error(`Failed to send doctor welcome email to ${email}`);
            return ok;
        }
        catch (error) {
            logger_utils_1.securityLogger.error(`Error sending doctor welcome email to ${email}:`, error);
            return false;
        }
    }
    async sendAssistantWelcomeEmail(email, firstName, lastName) {
        try {
            const subject = '¡Bienvenido a Qlinexa360! - Registro Exitoso';
            const htmlContent = `
        <!DOCTYPE html>
      ...
      `;
            const ok = await (0, email_utils_1.sendEmailHtml)(email, subject, htmlContent, email_utils_1.fromAddresses.noReply);
            if (ok)
                logger_utils_1.securityLogger.info(`Assistant welcome email sent to ${email}`);
            else
                logger_utils_1.securityLogger.error(`Failed to send assistant welcome email to ${email}`);
            return ok;
        }
        catch (error) {
            logger_utils_1.securityLogger.error(`Error sending assistant welcome email to ${email}:`, error);
            return false;
        }
    }
    async sendPatientWelcomeEmail(email, firstName, lastName) {
        try {
            const subject = '¡Bienvenido a Qlinexa360! - Registro Exitoso';
            const htmlContent = `
        <!DOCTYPE html>
      ...
      `;
            const ok = await (0, email_utils_1.sendEmailHtml)(email, subject, htmlContent, email_utils_1.fromAddresses.noReply);
            if (ok)
                logger_utils_1.securityLogger.info(`Patient welcome email sent to ${email}`);
            else
                logger_utils_1.securityLogger.error(`Failed to send patient welcome email to ${email}`);
            return ok;
        }
        catch (error) {
            logger_utils_1.securityLogger.error(`Error sending patient welcome email to ${email}:`, error);
            return false;
        }
    }
    async sendPasswordResetEmail(email, firstName, lastName, token) {
        try {
...
Equipo Qlinexa360`;
      ...
      `;
            const ok = await (0, email_utils_1.sendEmailHtmlWithTextFallback)(email, subject, htmlContent, textContent, email_utils_1.fromAddresses.noReply);
            if (ok)
                logger_utils_1.securityLogger.info(`Password reset email sent to ${email}`);
            else
                logger_utils_1.securityLogger.error(`Failed to send password reset email to ${email}`);
            return ok;
        }
        catch (error) {
            logger_utils_1.securityLogger.error(`Error sending password reset email to ${email}:`, error);
            return false;
        }
    }
    async sendPasswordChangedEmail(email, firstName, lastName) {
        try {
      ...
      `;
            const ok = await (0, email_utils_1.sendEmailHtml)(email, subject, htmlContent, email_utils_1.fromAddresses.noReply);
            if (ok)
                logger_utils_1.securityLogger.info(`Password changed confirmation email sent to ${email}`);
            else
                logger_utils_1.securityLogger.error(`Failed to send password changed confirmation email to ${email}`);
            return ok;
        }
        catch (error) {
            logger_utils_1.securityLogger.error(`Error sending password changed confirmation email to ${email}:`, error);
            return false;
        }
    }
}
...
class NotificationService {
    ...
    async sendTwoFactorRecoveryEmail(email, firstName, code) {
        try {
      ...
      `;
            const ok = await (0, email_utils_1.sendEmailHtml)(email, subject, htmlContent, email_utils_1.fromAddresses.noReply);
            if (ok)
                logger_utils_1.securityLogger.info(`Two-factor recovery email sent to ${email}`);
            else
                logger_utils_1.securityLogger.error(`Failed to send two-factor recovery email to ${email}`);
            return ok;
        }
        catch (error) {
            logger_utils_1.securityLogger.error(`Error sending two-factor recovery email to ${email}:`, error);
            return false;
        }
    }
    async sendInvitation(phone, email, doctorName, invitationUrl) {
    ...
    async sendPasswordSetupEmail(email, doctorName, passwordResetUrl) {
        try {
            const subject = 'Paciente registrado en Qlinexa360';
            const htmlContent = `
        <!DOCTYPE html>
      ...
      `;
            const ok = await (0, email_utils_1.sendEmailHtml)(email, subject, htmlContent, email_utils_1.fromAddresses.noReply);
            if (ok)
                logger_utils_1.securityLogger.info(`Password setup email sent to ${email}`);
            else
                logger_utils_1.securityLogger.error(`Failed to send password setup email to ${email}`);
            return ok;
        }
        catch (error) {
            logger_utils_1.securityLogger.error(`Error sending password setup email to ${email}:`, error);
            return false;
        }
    }
    async sendAssistantInvitation(email, doctorName, invitationUrl) {
    ...
    async sendExternalCollaborationInvite(params) {
        ...
        try {
            const emailSent = await this.emailService.sendExternalCollaborationInviteEmail(email, inviterName, patientName, padecimiento, websiteUrl, inviterEmail, avisoPdfBuffer);
            return { emailSent };
        }
        catch (error) {
            logger_utils_1.securityLogger.error('Error sending external collaboration invite', error);
    ...
    async sendInternalCollaborationInvite(params) {
        ...
        try {
            const emailSent = await this.emailService.sendInternalCollaborationInviteEmail(params.email, params.inviterName, params.patientName, params.padecimiento, websiteUrl, params.avisoPdfBuffer);
            return { emailSent };
        }
        catch (error) {
            logger_utils_1.securityLogger.error('Error sending internal collaboration invite', error);
            return { emailSent: false };
        }
    }
    async sendRecipeToPatientEmail(params) {
        ...
        <html><body style="font-family: Arial, sans-serif; color: #333;">
          ...
          <p>Gracias por usar Qlinexa360.</p>
            ...
            logger_utils_1.securityLogger.error('Error sending recipe to patient email:', error);
            return false;
        }
    }
    async sendInvoiceToPatientEmail(params) {
        ...
        <body style="font-family: Arial, sans-serif; color: #333; line-height: 1.6; margin: 0; padding: 0; background-color: #f5f5f5;">
          <div style="max-width: 600px; margin: 0 auto; padding: 20px; background-color: #ffffff;">
            ...
            const ok = await (0, email_utils_1.sendEmailHtml)(params.toEmail, subject, html, email_utils_1.fromAddresses.noReply, attachments);
            return ok;
        }
        catch (error) {
            logger_utils_1.securityLogger.error('Error sending invoice to patient email:', error);
            return false;
        }
    }
    ...
    static async sendDoctorNotification(email, phone, appointmentData) {
        const svc = NotificationService.getInstance();
        const [emailSent, whatsappSent] = await Promise.all([
    ...
    static async sendAppointmentReminder(email, phone, appointmentData) {
        const svc = NotificationService.getInstance();
        const [emailSent, whatsappSent] = await Promise.all([
    ...
    static async sendWelcomeEmail(email, firstName, lastName, role) {
        const svc = NotificationService.getInstance();
        try {
            let emailSent = false;
            switch (role) {
            ...
            logger_utils_1.securityLogger.error(`Error sending welcome email to ${email} for role ${role}:`, error);
            return { emailSent: false };
        }
    }
    static async sendPasswordResetEmail(email, firstName, lastName, token) {
        const svc = NotificationService.getInstance();
        try {
    ...
    static async sendPasswordChangedEmail(email, firstName, lastName) {
        const svc = NotificationService.getInstance();
        try {
            ...
            logger_utils_1.securityLogger.error(`Error sending password changed email to ${email}:`, error);
            return { emailSent: false };
        }
    }
    // Enviar email de confirmación de cita
    static async sendAppointmentConfirmationEmail(patientEmail, patientFirstName, patientLastName, appointmentDate, doctorFirstName, doctorLastName, confirmationToken, reminderType, timezone) {
        try {
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="padding: 30px; background: #f8f9fa;">
            ...
            <div style="background: white; border: 2px solid #e9ecef; border-radius: 10px; padding: 20px; margin: 20px 0;">
              ...
              <p style="color: #555; margin: 5px 0; font-size: 16px;">
                <strong>Fecha:</strong> ${(0, date_utils_1.formatAppointmentDate)(appointmentDate, timezone)}
              </p>
              <p style="color: #555; margin: 5px 0; font-size: 16px;">
                <strong>Hora:</strong> ${(0, date_utils_1.formatAppointmentTime)(appointmentDate, timezone)}
              </p>
              <p style="color: #555; margin: 5px 0; font-size: 16px;">
      ...
      `;
            const result = await (0, email_utils_1.sendEmailHtml)(patientEmail, `Confirmación de Cita - ${reminderText}`, emailContent, email_utils_1.fromAddresses.noReply);
            return result;
        }
        catch (error) {
            logger_utils_1.securityLogger.error('Error al enviar email de confirmación de cita:', error);
            return false;
        }
    }
    // Obtener texto descriptivo del tipo de recordatorio
    static getReminderTypeText(reminderType) {
        switch (reminderType) {
    ...
    async sendPreConsultationLink(email, phone, link, doctorName, appointmentDate) {
        const [emailSent, whatsappSent] = await Promise.all([
    ...
    async sendPreConsultationCompletedNotification(email, phone, patientName, appointmentDate) {
        const [emailSent, whatsappSent] = await Promise.all([
