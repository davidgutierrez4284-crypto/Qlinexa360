...
exports.createCaseMedicalRecord = exports.listCaseMedicalRecords = exports.deleteClinicalCase = exports.updateClinicalCase = exports.getClinicalCase = exports.createClinicalCase = exports.listClinicalCases = void 0;
...
const getDoctorAccessForPatient = async (doctorId, patientId, userId) => {
    ...
    if (userId) {
        ...
        if ((patient === null || patient === void 0 ? void 0 : patient.userId) === userId) {
            return { isCollaborator: false, collaborativeCaseIds: [] };
        }
    }
    ...
    const collaborations = await database_1.default.padecimientoDoctorColaborador.findMany({
        where: {
            doctorId,
            patientId
        },
    ...
    if (collaborations.length === 0) {
        throw new error_utils_1.AppError('No tienes acceso a los casos clínicos de este paciente.', 403);
    }
    return {
        isCollaborator: true,
...
const listClinicalCases = async (req, res) => {
    var _a;
    try {
        const { patientId } = req.params;
        const doctorId = await resolveDoctorId(req);
        const { isCollaborator, collaborativeCaseIds } = await getDoctorAccessForPatient(doctorId, patientId, (_a = req.user) === null || _a === void 0 ? void 0 : _a.userId);
        const cases = await database_1.default.clinicalCase.findMany({
            where: Object.assign({ patientId }, (isCollaborator ? { id: { in: collaborativeCaseIds } } : {})),
        ...
        res.json(cases.map(c => (Object.assign(Object.assign({}, c), { padecimiento: c.padecimiento }))));
    }
    catch (error) {
        const handled = error instanceof error_utils_1.AppError ? error : new error_utils_1.AppError('Error al obtener casos clínicos', 500);
        res.status(handled.statusCode).json({ message: handled.message });
    }
};
exports.listClinicalCases = listClinicalCases;
...
const createClinicalCase = async (req, res) => {
    try {
        ...
        if (!padecimiento)
            throw new error_utils_1.AppError('El padecimiento es obligatorio', 400);
        if (padecimiento.length > 20)
            throw new error_utils_1.AppError('El padecimiento debe tener máximo 20 caracteres', 400);
        const clinicalCase = await database_1.default.clinicalCase.create({
            data: {
                patientId,
                padecimiento,
        ...
        const handled = error instanceof error_utils_1.AppError ? error : new error_utils_1.AppError('Error al crear caso clínico', 500);
        res.status(handled.statusCode).json({ message: handled.message });
    }
};
exports.createClinicalCase = createClinicalCase;
// Obtener un caso clínico específico
const getClinicalCase = async (req, res) => {
    try {
        const { caseId } = req.params;
        const doctorId = await resolveDoctorId(req);
        const clinicalCase = await database_1.default.clinicalCase.findUnique({
            where: { id: caseId },
            include: {
                medicalRecords: {
                    orderBy: [{ date: 'desc' }, { createdAt: 'desc' }],
        ...
        if (isCollaborator && !collaborativeCaseIds.includes(clinicalCase.id)) {
            throw new error_utils_1.AppError('No tienes acceso a este caso clínico.', 403);
        }
        res.json(clinicalCase);
    }
    catch (error) {
        const handled = error instanceof error_utils_1.AppError ? error : new error_utils_1.AppError('Error al obtener caso clínico', 500);
        res.status(handled.statusCode).json({ message: handled.message });
    }
};
exports.getClinicalCase = getClinicalCase;
// Actualizar estado o padecimiento de un caso clínico
const updateClinicalCase = async (req, res) => {
    try {
        const { caseId } = req.params;
        const { padecimiento, status } = req.body;
        if (typeof padecimiento === 'string') {
            ...
            if (!trimmed) {
                throw new error_utils_1.AppError('El padecimiento no puede estar vacío', 400);
            }
            if (trimmed.length > 20) {
                throw new error_utils_1.AppError('El padecimiento debe tener máximo 20 caracteres', 400);
            }
        }
        const updated = await database_1.default.clinicalCase.update({
            where: { id: caseId },
            data: Object.assign(Object.assign({}, (padecimiento && { padecimiento: padecimiento.trim() })), (status && { status }))
        });
        res.json(updated);
    }
    catch (error) {
...
exports.updateClinicalCase = updateClinicalCase;
// Eliminar un caso clínico únicamente si no tiene consultas asociadas
const deleteClinicalCase = async (req, res) => {
    try {
        const { caseId } = req.params;
        const existing = await database_1.default.clinicalCase.findUnique({ where: { id: caseId } });
        if (!existing) {
            return res.status(404).json({ message: 'Caso clínico no encontrado' });
        }
        const recordsCount = await database_1.default.medicalRecord.count({ where: { clinicalCaseId: caseId } });
        if (recordsCount > 0) {
            throw new error_utils_1.AppError('No se puede eliminar el caso clínico porque tiene consultas asociadas.', 400);
        }
        await database_1.default.clinicalCase.delete({ where: { id: caseId } });
        res.json({ success: true, message: 'Caso clínico eliminado correctamente.' });
    }
    catch (error) {
        const handled = error instanceof error_utils_1.AppError ? error : new error_utils_1.AppError('Error al eliminar caso clínico', 500);
        res.status(handled.statusCode).json({ message: handled.message });
    }
};
exports.deleteClinicalCase = deleteClinicalCase;
// Listar notas clínicas de un caso
const listCaseMedicalRecords = async (req, res) => {
    try {
        const { caseId } = req.params;
        const doctorId = await resolveDoctorId(req);
        const clinicalCase = await database_1.default.clinicalCase.findUnique({
        ...
        const records = await database_1.default.medicalRecord.findMany({
        ...
        records.forEach((record, index) => {
            ...
            console.log(`Registro ${index + 1}:`, {
                ...
                date: record.date,
                dateISO: (_a = record.date) === null || _a === void 0 ? void 0 : _a.toISOString(),
                dateLocal: (_b = record.date) === null || _b === void 0 ? void 0 : _b.toLocaleDateString('es-ES')
            });
        });
        res.json(records);
    }
    catch (error) {
        const handled = error instanceof error_utils_1.AppError ? error : new error_utils_1.AppError('Error al obtener notas clínicas del caso', 500);
        res.status(handled.statusCode).json({ message: handled.message });
    }
};
exports.listCaseMedicalRecords = listCaseMedicalRecords;
// Crear una nota clínica dentro de un caso
const createCaseMedicalRecord = async (req, res) => {
    try {
        if (!req.user)
            throw new error_utils_1.AppError('Autenticación requerida.', 401);
        const { caseId } = req.params;
        const { doctorPatientId, patientId, diagnosis, treatment, notes, tags, clinicalEvolution, formData, fileIds, links, date } = req.body;
        if (!diagnosis || !clinicalEvolution)
            throw new error_utils_1.AppError('Diagnóstico y evolución clínica son obligatorios', 400);
        ...
        const record = await database_1.default.medicalRecord.create({
            data: {
                ...
                files: fileIds && fileIds.length > 0 ? { connect: fileIds.map((id) => ({ id })) } : undefined,
        ...
        await database_1.default.medicalRecord.updateMany({
            where: {
                clinicalCaseId: caseId,
        ...
        const handled = error instanceof error_utils_1.AppError ? error : new error_utils_1.AppError('Error al crear nota clínica en el caso', 500);
        res.status(handled.statusCode).json({ message: handled.message });
    }
};
exports.createCaseMedicalRecord = createCaseMedicalRecord;
