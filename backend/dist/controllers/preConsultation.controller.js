...
exports.uploadFileForPreConsultation = exports.validatePreConsultationToken = exports.getPreConsultationByAppointment = exports.completePreConsultation = exports.savePreConsultationData = exports.getPreConsultationByToken = exports.generatePreConsultationLink = exports.createPreConsultationForAppointment = void 0;
...
const generatePreConsultationToken = () => {
 ...
 */
const createPreConsultationForAppointment = async (appointmentId) => {
    var _a;
    try {
        ...
        if ((doctor === null || doctor === void 0 ? void 0 : doctor.user) && typeof doctor.user === 'object' && 'firstName' in doctor.user && 'lastName' in doctor.user) {
            doctorName = `${doctor.user.firstName} ${doctor.user.lastName}`;
        }
        else if ((_a = appointment.doctor) === null || _a === void 0 ? void 0 : _a.userId) {
        ...
        try {
            ...
            const result = await notificationService.sendPreConsultationLink(appointment.patient.user.email || '', appointment.patient.user.phone || '', link, doctorName, appointmentDate);
            ...
            if (notificationError === null || notificationError === void 0 ? void 0 : notificationError.stack) {
...
exports.createPreConsultationForAppointment = createPreConsultationForAppointment;
/**
 * Generar link de pre-consulta para una cita
 * Solo doctores pueden generar estos links
 */
const generatePreConsultationLink = async (req, res) => {
    var _a;
    try {
        const { appointmentId } = req.params;
        const user = req.user;
        if (!user || (user.role !== 'DOCTOR' && user.role !== 'ASISTENTE')) {
            throw new error_utils_1.AppError('Solo profesionales de la salud pueden generar links de pre-consulta', 403);
        }
        ...
        if (user.role === 'DOCTOR') {
            ...
            if (!doctor || doctor.id !== appointment.doctorId) {
                throw new error_utils_1.AppError('No tienes permisos para acceder a esta cita', 403);
            }
        }
        // Verificar si ya existe una pre-consulta para esta cita
        ...
        try {
            await notificationService.sendPreConsultationLink(appointment.patient.email || '', appointment.patient.phone || '', link, doctorName, appointmentDate);
        }
        catch (notificationError) {
        ...
        const statusCode = error instanceof error_utils_1.AppError ? error.statusCode : 500;
...
exports.getPreConsultationByToken = getPreConsultationByToken;
/**
 * Guardar datos de pre-consulta (el paciente llena el formulario)
 */
const savePreConsultationData = async (req, res) => {
    try {
        const { token } = req.params;
...
exports.savePreConsultationData = savePreConsultationData;
/**
 * Marcar pre-consulta como completada
 */
const completePreConsultation = async (req, res) => {
    try {
        const { token } = req.params;
        const { formData } = req.body;
        if (!formData || typeof formData !== 'object') {
        ...
        try {
            ...
            const patientNotes = (formData === null || formData === void 0 ? void 0 : formData.notes) || '';
            ...
            const hasFiles = (formData === null || formData === void 0 ? void 0 : formData.files) && ((Array.isArray(formData.files.PATIENT_PHOTO) && formData.files.PATIENT_PHOTO.length > 0) ||
            ...
            const medicalRecord = await prisma.medicalRecord.create({
                data: {
                    ...
                    reason: (formData === null || formData === void 0 ? void 0 : formData.reason) || 'Pre-consulta inicial',
                    tags: (formData === null || formData === void 0 ? void 0 : formData.tags) && Array.isArray(formData.tags) ? formData.tags : ['pre-consulta', 'evaluación-inicial'],
                    clinicalEvolution: (formData === null || formData === void 0 ? void 0 : formData.clinicalEvolution) || 'INITIAL_EVALUATION',
                    formData: formData, // Guardar todos los datos del formulario
                    date: preConsultation.completedAt || new Date(),
                    isPublic: (formData === null || formData === void 0 ? void 0 : formData.isPublic) !== undefined ? formData.isPublic : true,
                    isComplete: false,
        ...
        const notificationService = notification_service_1.NotificationService.getInstance();
        try {
            await notificationService.sendPreConsultationCompletedNotification(preConsultation.appointment.doctor.user.email || '', preConsultation.appointment.doctor.user.phone || '', preConsultation.appointment.patient.user.firstName + ' ' + preConsultation.appointment.patient.user.lastName, preConsultation.appointment.date);
        }
        catch (notificationError) {
...
exports.completePreConsultation = completePreConsultation;
/**
 * Obtener pre-consultas de una cita (para el doctor)
 */
const getPreConsultationByAppointment = async (req, res) => {
    try {
        const { appointmentId } = req.params;
        const user = req.user;
        if (!user || (user.role !== 'DOCTOR' && user.role !== 'ASISTENTE')) {
            throw new error_utils_1.AppError('Solo profesionales de la salud pueden ver pre-consultas', 403);
        }
        const preConsultation = await prisma.preConsultation.findUnique({
            where: { appointmentId },
            include: {
...
exports.getPreConsultationByAppointment = getPreConsultationByAppointment;
/**
 * Middleware: validar token de pre-consulta y adjuntar preConsultation al request
 */
const validatePreConsultationToken = async (req, res, next) => {
    try {
        const { token } = req.params;
        if (!token) {
        ...
        req.preConsultation = preConsultation;
        next();
    }
    catch (error) {
...
exports.validatePreConsultationToken = validatePreConsultationToken;
/**
 * Subir archivo en pre-consulta (sin autenticación JWT - usa token de pre-consulta)
 */
const uploadFileForPreConsultation = async (req, res) => {
    try {
        const file = req.file;
        const preConsultation = req.preConsultation;
        ...
        const { url } = await (0, file_utils_1.uploadToS3)(file, category, patientUserId);
        res.status(201).json({
...
exports.uploadFileForPreConsultation = uploadFileForPreConsultation;
