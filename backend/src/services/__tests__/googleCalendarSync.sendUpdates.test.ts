...
import { GoogleCalendarSyncService, GoogleCalendarNotReadyError } from '../googleCalendarSync.service';
import { resolveGoogleCalendarSendUpdates } from '../../utils/calendarSync.utils';
...
describe('GoogleCalendarSyncService sendUpdates integration', () => {
  it('insert uses sendUpdates all by default when creating with attendees', async () => {
    await GoogleCalendarSyncService.upsertEvent('doc-1', basePayload);
  ...
  it('update defaults to sendUpdates none when not explicitly requested', async () => {
    await GoogleCalendarSyncService.upsertEvent('doc-1', {
      ...basePayload,
  ...
  it('throws GoogleCalendarNotReadyError when doctor has no google config (no silent null)', async () => {
    ...
    await expect(GoogleCalendarSyncService.upsertEvent('doc-1', basePayload)).rejects.toBeInstanceOf(
      GoogleCalendarNotReadyError
    );
