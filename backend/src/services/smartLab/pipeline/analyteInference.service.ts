...
function unitToken(unit: string | null | undefined): string {
  return (unit ?? '').toLowerCase().replace(/µ/g, 'μ').trim();
}
 ...
 */
export function inferAnalyteFromUnitRange(candidate: ParameterCandidate): ParameterCandidate {
  ...
  if (
    (/^sodio$/i.test(name) || /^potasio$/i.test(name) || /^cloro$/i.test(name)) &&
    !/meq|mmol/.test(u)
  ) {
    return {
      ...candidate,
