...
import { fetchBufferFromUrl, getS3SignedUrlIfExists } from '../utils/file.utils';
import {
  fromAddresses,
  getLastSmtpError,
  isSmtpConfigured,
...
export class RecipeController {
   ...
   */
  static async uploadRecipeTemplate(req: Request, res: Response) {
    try {
      ...
      securityLogger.info(`Template de receta creado para doctor ${doctorId}`, { templateId: template.id });
      ...
      securityLogger.error('Error al crear template de receta:', error);
      res.status(500).json({
        success: false,
   ...
   */
  static async getRecipeTemplates(req: Request, res: Response) {
    try {
      const { doctorId } = req.params;
      ...
      securityLogger.error('Error al obtener templates de recetas:', error);
      res.status(500).json({
        success: false,
        message: 'Error interno del servidor'
      });
  ...
  static async createRecipe(req: Request, res: Response) {
    try {
      ...
      const {
        doctorId,
        pacienteId,
        citaId, // Ahora se refiere a consultationId
        observaciones,
      ...
      if (esRecetaMedicamento && medicamentos.length > 0) {
        ...
        })).filter((m: { medicamento: string }) => m.medicamento);
      ...
      try {
        ...
        pdfFileName = await RecipePdfService.generateRecipePdf(receta.id);
      ...
      if (!isS3Url) {
        ...
        if (fsLocal.existsSync(localPath)) fileSize = fsLocal.statSync(localPath).size;
      }
      ...
      // ENVIAR CORREO AUTOMÁTICAMENTE AL PACIENTE
       try {
         ...
         if (patientInfo && (patientInfo.email || patientInfo.user?.email)) {
           const patientEmail = patientInfo.email || patientInfo.user?.email;
           const patientName = `${patientInfo.firstName} ${patientInfo.lastName}`;
           const doctorName = `${recetaCompleta.doctor?.user.firstName || ''} ${recetaCompleta.doctor?.user.lastName || ''}`.trim();
           
           // Construir URL de visualización
           const emissionDate = recetaCompleta.fechaEmision || new Date();
           const viewUrl = RecipePdfService.buildPdfViewUrl(
             recetaCompleta.id,
             doctorId,
             emissionDate
           );
          ...
          if (emailSent) {
            ...
            securityLogger.info(`Receta médica creada y enviada por correo: ${receta.id}`, {
              doctorId,
              pacienteId,
              citaId,
              patientEmail
            ...
            securityLogger.warn(`Receta médica creada pero falló envío de correo: ${receta.id}`, {
              doctorId,
              pacienteId,
              citaId,
              patientEmail
          ...
          securityLogger.warn(`Receta médica creada pero paciente sin email: ${receta.id}`, {
            doctorId,
            pacienteId,
            citaId
          });
        ...
        securityLogger.error(`Error enviando correo automático de receta: ${receta.id}`, emailError);
        // No fallar la creación de la receta por un error de correo
      }

      securityLogger.info(`Receta médica creada: ${receta.id}`, {
        doctorId,
        pacienteId,
        citaId
      });
      ...
      securityLogger.error('Error al crear receta médica:', error);
      res.status(500).json({
        success: false,
        message: 'Error interno del servidor'
      });
  ...
  static async getMyRecipes(req: Request, res: Response) {
    try {
      ...
      securityLogger.error('Error al obtener recetas del doctor:', error);
      res.status(500).json({
        success: false,
        message: 'Error interno del servidor'
      });
   ...
   */
  static async getPatientRecipes(req: Request, res: Response) {
    try {
      ...
      securityLogger.error('Error al obtener recetas del paciente:', error);
      res.status(500).json({
        success: false,
        message: 'Error interno del servidor'
      });
   ...
   */
  static async getDoctorRecipes(req: Request, res: Response) {
    try {
      const { doctorId } = req.params;
      const { limit = 50, offset = 0 } = req.query;
      ...
      const missingPatientIds = recetas
        .filter((r: any) => !r?.consulta?.clinicalCase?.padecimiento)
        .map((r: any) => r.paciente.id);

      let patientIdToPadecimiento: Record<string, string> = {};
      ...
      const recetasWithDerived = recetas.map((r: any) => ({
        ...r,
        padecimiento: r?.consulta?.clinicalCase?.padecimiento || patientIdToPadecimiento[r?.paciente?.id] || ''
      }));
   ...
   */
  static async testSmtpConnection(req: AuthRequest, res: Response) {
    try {
      if (!req.user?.userId) {
      ...
      const sent = await sendEmailHtml(
        user.email,
        'Prueba SMTP - Qlinexa360',
        '<p>Si recibes este correo, el envío SMTP del servidor funciona correctamente.</p>',
        fromAddresses.noReply
      );

      if (!sent) {
        const detail = getLastSmtpError() || 'Error desconocido al enviar';
        securityLogger.error('SMTP test send failed:', detail);
      ...
      securityLogger.error('Error en test SMTP:', error);
   ...
   */
  static async emailRecipeToPatient(req: AuthRequest, res: Response) {
    try {
      ...
      const patientEmail = recipe.paciente?.email || recipe.paciente?.user?.email || null;
      if (!patientEmail) {
      ...
      try {
        ...
        if (tmpRes) {
          ...
          viewUrl = RecipePdfService.buildPdfViewUrl(tmpRes.id, tmpRes.doctorId, emissionDate);
          ...
          const sent = await svc.sendRecipeToPatientEmail({
            ...
            doctorName: `${recipe.doctor?.user.firstName || ''} ${recipe.doctor?.user.lastName || ''}`.trim(),
            recipeId: recipe.id,
            viewUrl
          });
          if (!sent) {
            const detail = getLastSmtpError() || 'No se pudo enviar el correo';
            securityLogger.error('Fallo SMTP al enviar receta:', detail);
        ...
        securityLogger.error('Error al enviar receta por email:', error);
  ...
  static async getRecipeById(req: Request, res: Response) {
    try {
      ...
      securityLogger.error('Error al obtener receta:', error);
      res.status(500).json({
        success: false,
        message: 'Error interno del servidor'
      });
   ...
   */
  static async getRecipePdfViewUrl(req: AuthRequest, res: Response) {
    try {
      const { id } = req.params;
      const user = req.user;
      ...
      if (user.role === 'PATIENT') {
        ...
        if (receta.pacienteId !== patient.id) {
          securityLogger.security(
            'Unauthorized recipe access attempt by patient',
            { recipeId: id, requestedBy: user.userId, recipePatientId: receta.pacienteId, patientId: patient.id },
            user.userId,
            req.ip
          );
          return res.status(403).json({
      ...
      securityLogger.error('Error al generar URL de visualización de PDF:', error);
      res.status(500).json({
        success: false,
        message: 'Error interno del servidor'
      });
   ...
   */
  static async serveRecipePdf(req: Request, res: Response) {
    try {
      const { id } = req.params;
      ...
      const expectedHash = RecipePdfService.generateProductionHash(receta.id, receta.doctorId, emissionDate);
      ...
      if (receta.archivoPdf.startsWith('http')) {
        try {
          ...
          const signed = await getS3SignedUrlIfExists(receta.archivoPdf, maxPresign);
          if (signed) {
        ...
        try {
          const { buffer } = await fetchBufferFromUrl(receta.archivoPdf);
      ...
      if (!fsLocal.existsSync(filePath)) {
        ...
        try {
          const regenerated = await RecipePdfService.generateRecipePdf(id);
          if (typeof regenerated === 'string' && regenerated.startsWith('http')) {
            const { buffer } = await fetchBufferFromUrl(regenerated);
            res.setHeader('Content-Type', 'application/pdf');
            res.setHeader('Content-Disposition', 'inline; filename="receta.pdf"');
            res.setHeader('Cache-Control', 'public, max-age=3600');
            return res.send(buffer);
      ...
      securityLogger.info(`PDF servido para visualización: ${id}`);
    } catch (error) {
      console.error('Error sirviendo PDF:', error);
      securityLogger.error('Error al servir PDF de receta:', error);
      res.status(500).json({
        success: false,
        message: 'Error interno del servidor'
      });
   ...
   */
  static async downloadRecipePdf(req: Request, res: Response) {
    try {
      const { id } = req.params;
      ...
      const result = await RecipePdfService.generateRecipePdf(id);

      if (typeof result === 'string' && result.startsWith('http')) {
        try {
          const { buffer } = await fetchBufferFromUrl(result);
          res.setHeader('Content-Type', 'application/pdf');
          res.setHeader('Content-Disposition', `attachment; filename="receta_${id}.pdf"`);
          return res.send(buffer);
        } catch (fetchErr) {
      ...
      securityLogger.info(`PDF descargado para receta: ${id}`);
    } catch (error) {
      console.error('Error generando PDF:', error);
      securityLogger.error('Error al generar PDF de receta:', error);
      res.status(500).json({
        success: false,
        message: 'Error interno del servidor'
      });
  ...
  static async updateRecipe(req: Request, res: Response) {
    try {
      ...
      securityLogger.info(`Receta médica actualizada: ${id}`);
      ...
      securityLogger.error('Error al actualizar receta médica:', error);
      res.status(500).json({
        success: false,
        message: 'Error interno del servidor'
      });
  ...
  static async deleteRecipe(req: Request, res: Response) {
    try {
      ...
      securityLogger.info(`Receta médica eliminada: ${id}`);
      ...
      securityLogger.error('Error al eliminar receta médica:', error);
      res.status(500).json({
        success: false,
        message: 'Error interno del servidor'
      });
   ...
   */
  static async verifyRecipe(req: Request, res: Response) {
    try {
      const { id } = req.params;
      ...
      if (isBrowserRequest && receta.archivoPdf) {
        const pdfViewUrl = RecipePdfService.buildPdfViewUrl(receta.id, receta.doctorId, emissionDate);
   ...
   */
  static async getRecipeStats(req: Request, res: Response) {
    try {
      const { doctorId } = req.params;
      ...
      securityLogger.error('Error al obtener estadísticas de recetas:', error);
      res.status(500).json({
        success: false,
        message: 'Error interno del servidor'
      });
   ...
   */
  static async searchPatientsForRecipes(req: Request, res: Response) {
    try {
      ...
      let collaborativePatientsQuery: any = {
        AND: [
          {
            clinicalCases: { 
              some: { 
      ...
      const dbg = allPatients.find((p: any) => p.user?.email === 'paciente@test.com');
      if (dbg) {
        console.log('=== DIAGNÓSTICO RECETAS paciente@test.com ===');
        console.log('doctorsIds:', (dbg.doctors || []).map((d: any) => d.doctorId));
        console.log('clinicalCases:', (dbg.clinicalCases || []).map((c: any) => ({ id: c.id, padecimiento: c.padecimiento })));
      }

      // Formatear respuesta
      const formattedPatients = allPatients.map((patient: any) => {
        // Verificar si el doctor actual está en la lista de doctors de este paciente
        const isTitular = patient.doctors && patient.doctors.some((d: any) => d.doctorId === doctorId);
        ...
        return {
          ...
          clinicalCases: patient.clinicalCases.map((case_: any) => ({
            id: case_.id,
      ...
      securityLogger.error('Error al buscar pacientes para recetas:', error);
      res.status(500).json({
        success: false,
        message: 'Error interno del servidor'
      });
