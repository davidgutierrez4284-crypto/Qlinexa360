...
export const submitFeedback = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!type || !message) {
      throw new AppError('Tipo y mensaje son requeridos.', 400);
    }

    if (type !== 'sugerencia' && type !== 'queja') {
      throw new AppError('Tipo inválido. Debe ser "sugerencia" o "queja".', 400);
    }

    let userInfo: { fullName: string; email: string; phone: string; source: 'enrolled' | 'external' };
    ...
    if (req.user) {
      ...
      if (!emailTrimmed || !emailTrimmed.includes('@')) {
        throw new AppError('Correo electrónico requerido para enviar sugerencias o quejas sin sesión iniciada.', 400);
      }
      userInfo = {
    ...
    if (!emailSent) {
      throw new AppError('Error al enviar el correo. Por favor intenta más tarde.', 500);
    }
